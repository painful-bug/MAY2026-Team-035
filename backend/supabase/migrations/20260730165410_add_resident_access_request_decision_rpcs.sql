-- The legacy project has an unrelated invoice-oriented overload of
-- approve_access_request.  This overload is the resident-onboarding contract
-- used by the BFF; keeping both signatures avoids breaking legacy callers.
...
begin
  ...
  ) on conflict (community_id, profile_id) where ended_at is null
    do nothing
  ...
  if target_unit_id is not null then
    ...
    ) on conflict (unit_id, membership_id) where ended_at is null do nothing;
  end if;
...
revoke all on function public.reject_access_request(uuid, uuid, text)
  from public, anon, authenticated;
grant execute on function public.approve_access_request(uuid, uuid, uuid, public.residency_relationship)
  to service_role;
grant execute on function public.reject_access_request(uuid, uuid, text)
  to service_role;
