...
| `20260823120000_complaint_engine_v2_repairs.sql` | forward-only reconciliation of the Complaint Engine v2 repair branch: casts manual-window queue priority, resolves the assignment trigger's row shape, makes declined-candidate overrides explicit, and lets critical force assignment run inside a worker decline transaction while preserving the current `/worker?job=` route. The branch's backdated `20260817142354` and `20260817142820` files are intentionally not imported because hosted history was already verified through `20260817144725`. **Not yet applied to hosted** (owner probe, 2026-08-23); accepted by the complaint-engine owner the same day |

**This table stops at `20260812120000` apart from the rows above.** It was not
...
numbered section for every one of them. The rows above are here because those
files arrived from another branch and their provenance needed somewhere to live
that a reader of this directory would find.
...
`20260823180000_resident_sets_the_time.sql` are the two such records discovered
during the 2026-08-23 reconciliation.
