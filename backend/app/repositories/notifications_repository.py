...
DEFINER functions in ``0030``. Both halves of that are load-bearing here in a way
they are not everywhere else in this codebase, because ``notifications`` is the
first table this workstream touches that carries an **RLS policy of its own**:

    notifications_read_own -- using (public.is_own_membership(recipient_membership_id))

So the ``recipient_membership_id`` filter below is not the security boundary the
way the community filter is on the amenity catalogue. It is there to ask a
narrower question than the policy already answers, and if it were deleted the
database would still refuse to return anyone else's rows. That is the right way
round: the API is what makes the query efficient, the database is what makes it
correct.
...
def list_feed(
    ...
    *,
    ...
    query = (
        ...
        .eq("recipient_membership_id", membership_id)
    )
    if unread_only:
...
def unread_count(client: Client, *, membership_id: str) -> int:
...
def mark_all_read(client: Client, *, membership_id: str) -> int:
    """Clear the badge (RPC). Returns how many rows moved."""
    try:
        response = client.rpc(
            "mark_all_notifications_read", {"p_membership_id": membership_id}
        ).execute()
    except Exception as exc:  # noqa: BLE001
        raise translate(
