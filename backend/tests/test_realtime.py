...
def test_dispatch_routes_each_row_to_its_own_community_only():
    async def body():
        hub = RealtimeHub()
        alpha, beta = _Subscriber(ALPHA), _Subscriber(BETA)
...
def test_every_subscriber_in_a_community_gets_the_same_event():
    ...
    async def body():
        hub = RealtimeHub()
        first, second = _Subscriber(ALPHA), _Subscriber(ALPHA)
...
def test_a_lagging_client_is_told_to_resync():
    ...
    event = realtime._resync_event(99)
...
def test_subscribe_yields_live_events_and_unregisters_on_exit():
    async def body():
        ...
        stream = hub.subscribe(ALPHA, last_event_id=0)
        frames: list[str] = []
...
def test_reconnecting_with_a_last_event_id_replays_the_gap():
    ...
    async def body():
        ...
        async def fake_backfill(community_id, last_event_id):
            seen["community_id"] = community_id
        ...
        stream = hub.subscribe(ALPHA, last_event_id=4)
...
def test_a_failing_poll_does_not_kill_the_loop():
    ...
    async def body():
        ...
        hub._subscribers = {ALPHA: {_Subscriber(ALPHA)}}
