...
export async function enablePush() {
  ...
  // A subscription is bound to the key that created it and the protocol has no
  // dual-key period, so a rotated key means an existing subscription is dead
  // rather than stale. Drop it and take a new one.
  ...
  // Posted exactly as the Push API handed it over. Transcribing the key pair by
  // hand is where `auth` ends up in the `p256dh` field, and that failure looks
  // like a push that silently never decrypts.
