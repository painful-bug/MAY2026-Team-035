...
|---|---|---|
| **F1** 🔴 | Applying **every** migration to a real Supabase project — `0001_baseline.sql` included, which has never run either — and running the verification queries in each file's footer. **Both build orders are now complete, so this is the whole remaining risk.** `0001`'s GIST exclusion constraint on `amenity_bookings` is the only thing standing between two residents and the same hall; `0031`'s SLA rule, `0032`'s code hashing and `0033`'s two settlement RPCs are the other three places where the database, not the API, is what makes a guarantee true. None of the four has ever executed. `0022` has one verification query that legitimately *can* return rows — stale `community_modules` keys with no catalogue entry, since the keys were seeded before the catalogue existed and no FK was added on purpose. *(Numbering: this item cited `0010`–`0017` until 2026-08-04; those files were quarantined and rebuilt onto the baseline as `0018`–`0023`, and the resident range added `0028`–`0033`.)* | backend + whoever holds the Supabase project |
