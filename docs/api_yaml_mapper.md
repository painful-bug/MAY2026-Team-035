...
*(**Re-counted 2026-08-23 on `live-app-fixes`, third pass — issue #48 amenity-defects docs sync:
still 210 operations across 179 paths** (no route changed; this pass documented behaviour, not
endpoints), all 210 in the spec, and `api_map_scan.py` now reporting **18** findings, not 20 — the
two `NOT IN API.md` findings for `PUT`/`DELETE /dashboard/amenities/{amenity_id}` are gone, because
those two operations gained a real `API.md` heading this pass (§3 below; §5.2 below no longer lists
them as having no reference section at all). All 24 stories still agreeing. §3's tables **were**
regenerated in this pass — the amenity-defects fix added one new `###` heading and rewrote a dozen
others inside §10 and §5, pushing roughly ninety `API.md` line references down; `regen_mapper.py`
resynced all of them, including the ones this pass had briefly hand-computed while writing them (both
agreed). Nothing in this pass touched a router, a handler or `openapi.yaml` — only prose, so `derived`
cells (route, handler line, `operationId`, success schema) were mechanically unchanged and only the
`API.md` line pointers moved.)*

*(**Re-counted 2026-08-24 on `live-app-fixes`: 211 operations across 180 paths**, all 211 in the
spec, `api_map_scan.py` still reporting the same **18** findings and all 24 stories agreeing —
`POST /work-orders/{workOrderId}/take-up` (ruling R8, supervisor take-up) added neither a finding
nor a story gap; it tags `US-2.8`, which was already served. §3's tables **were** regenerated in
this pass, and this time they had to be: the new `API.md` section sits inside §18 and pushed
roughly fifty line references below it down, and the new route moved two handler line numbers in
`work_orders.py`. Both kinds of drift are exactly what
[`regen_mapper.py`](../backend/scripts/regen_mapper.py) exists to resync.)*

*(**Re-counted 2026-08-28 on the residence-claim branch: still 211 operations across 180 paths** —
no route changed; the residence-claim migration (`20260828090000_residence_claim_on_join.sql`)
moved request/response shapes, not paths — all 211 in the spec, and `api_map_scan.py` now reporting
**10** findings, not 18: the three access-request decision routes gained
`AccessRequestDecisionResponse` (three `UNTYPED BODY` findings gone) and `API.md` §6 gained a
section for all seven access-request operations (five `NOT IN API.md` findings gone). All 24
stories still agreeing. §3's tables **were** regenerated — the new §6 sections pushed every
`API.md` reference below them down by roughly 190 lines, and the seven access-request rows' success
schemas and anchors are newly derived; see the 2026-08-28 row in §7 for the hand-blanking that let
the regenerator link them.)*
...
|---|---|---|---|---|
| `GET /health` | `health` :108 | `health_health_get` | 200 inline | § `GET /health` (API.md:359) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `POST /api/v1/access-requests` | `create_access_request` :23 | `create_access_request_api_v1_access_requests_post` | 201 AccessRequestResponse | § `POST /api/v1/access-requests` (API.md:1006) |
| `GET /api/v1/access-requests/mine` | `my_access_requests` :36 | `my_access_requests_api_v1_access_requests_mine_get` | 200 AccessRequestListResponse | § `GET /api/v1/access-requests/mine` (API.md:1043) |
| `POST /api/v1/access-requests/{request_id}/withdraw` | `withdraw_access_request` :43 | `withdraw_access_request_api_v1_access_requests__request_id__withdraw_post` | 200 AccessRequestResponse | § `POST /api/v1/access-requests/{request_id}/withdraw` (API.md:1050) |
| `GET /api/v1/admin/access-requests` | `admin_access_requests` :55 | `admin_access_requests_api_v1_admin_access_requests_get` | 200 AccessRequestListResponse | § `GET /api/v1/admin/access-requests` (API.md:1056) |
| `POST /api/v1/admin/access-requests/{request_id}/approve` | `approve_access_request` :64 | `approve_access_request_api_v1_admin_access_requests__request_id__approve_post` | 200 AccessRequestDecisionResponse | § `POST /api/v1/admin/access-requests/{request_id}/approve` (API.md:1069) |
| `POST /api/v1/admin/access-requests/{request_id}/blacklist` | `blacklist_access_request` :94 | `blacklist_access_request_api_v1_admin_access_requests__request_id__blacklist_post` | 200 AccessRequestDecisionResponse | § `POST /api/v1/admin/access-requests/{request_id}/blacklist` (API.md:1120) |
| `POST /api/v1/admin/access-requests/{request_id}/reject` | `reject_access_request` :79 | `reject_access_request_api_v1_admin_access_requests__request_id__reject_post` | 200 AccessRequestDecisionResponse | § `POST /api/v1/admin/access-requests/{request_id}/reject` (API.md:1112) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/amenities/{amenity_id}/approvals` | `list_approvals` :195 | `list_approvals_api_v1_amenities__amenity_id__approvals_get` | 200 Page_ApprovalRequest_ | § `GET /api/v1/amenities/{amenityId}/approvals` (API.md:2526) |
| `POST /api/v1/amenities/{amenity_id}/blocks` | `block_slot` :174 | `block_slot_api_v1_amenities__amenity_id__blocks_post` | 201 BookingSummary | § `POST /api/v1/amenities/{amenityId}/blocks` (API.md:2499) |
| `GET /api/v1/amenities/{amenity_id}/bookings` | `list_amenity_bookings` :67 | `list_amenity_bookings_api_v1_amenities__amenity_id__bookings_get` | 200 Page_BookingSummary_ | § `GET /api/v1/amenities/{amenityId}/bookings` (API.md:2317) |
| `POST /api/v1/amenities/{amenity_id}/bookings` | `create_admin_booking` :114 | `create_admin_booking_api_v1_amenities__amenity_id__bookings_post` | 201 BookingSummary | § `POST /api/v1/amenities/{amenityId}/bookings` (API.md:2401) |
| `POST /api/v1/amenities/{amenity_id}/bookings/request` | `request_booking` :142 | `request_booking_api_v1_amenities__amenity_id__bookings_request_post` | 201 Page_BookingSummary_ | § `POST /api/v1/amenities/{amenityId}/bookings/request` (API.md:2461) |
| `GET /api/v1/amenities/{amenity_id}/ledger` | `list_ledger` :335 | `list_ledger_api_v1_amenities__amenity_id__ledger_get` | 200 Page_LedgerTransaction_ | § `GET /api/v1/amenities/{amenityId}/ledger` (API.md:2660) |
| `GET /api/v1/amenities/{amenity_id}/ledger/summary` | `get_ledger_summary` :379 | `get_ledger_summary_api_v1_amenities__amenity_id__ledger_summary_get` | 200 LedgerSummary | § `GET /api/v1/amenities/{amenityId}/ledger/summary` (API.md:2749) |
| `POST /api/v1/amenity-bookings/cancel` | `cancel_bookings` :278 | `cancel_bookings_api_v1_amenity_bookings_cancel_post` | 200 MessageResult | § `POST /api/v1/amenity-bookings/cancel` (API.md:2606) |
| `POST /api/v1/amenity-bookings/{occurrence_id}/charges` | `add_charge` :475 | `add_charge_api_v1_amenity_bookings__occurrence_id__charges_post` | 201 LedgerTransaction | § `POST /api/v1/amenity-bookings/{occurrenceId}/charges` (API.md:2869) |
| `POST /api/v1/amenity-bookings/{occurrence_id}/damage` | `deduct_damage` :450 | `deduct_damage_api_v1_amenity_bookings__occurrence_id__damage_post` | 201 LedgerTransaction | § `POST /api/v1/amenity-bookings/{occurrenceId}/damage` (API.md:2848) |
| `POST /api/v1/amenity-bookings/{occurrence_id}/force-cancel` | `force_cancel_booking` :306 | `force_cancel_booking_api_v1_amenity_bookings__occurrence_id__force_cancel_post` | 200 BookingSummary | § `POST /api/v1/amenity-bookings/{occurrenceId}/force-cancel` (API.md:2640) |
| `POST /api/v1/amenity-bookings/{occurrence_id}/payments` | `record_payment` :401 | `record_payment_api_v1_amenity_bookings__occurrence_id__payments_post` | 201 LedgerTransaction | § `POST /api/v1/amenity-bookings/{occurrenceId}/payments` (API.md:2775) |
| `POST /api/v1/amenity-bookings/{occurrence_id}/refund` | `refund_deposit` :425 | `refund_deposit_api_v1_amenity_bookings__occurrence_id__refund_post` | 201 LedgerTransaction | § `POST /api/v1/amenity-bookings/{occurrenceId}/refund` (API.md:2815) |
| `POST /api/v1/amenity-bookings/{series_id}/approve` | `approve_booking` :235 | `approve_booking_api_v1_amenity_bookings__series_id__approve_post` | 200 Page_BookingSummary_ | § `POST /api/v1/amenity-bookings/{seriesId}/approve` (API.md:2570) |
| `POST /api/v1/amenity-bookings/{series_id}/reject` | `reject_booking` :255 | `reject_booking_api_v1_amenity_bookings__series_id__reject_post` | 200 Page_BookingSummary_ | § `POST /api/v1/amenity-bookings/{seriesId}/reject` (API.md:2585) |
| `GET /api/v1/amenity-reports` | `get_report` :504 | `get_report_api_v1_amenity_reports_get` | 200 AmenityReport | § `GET /api/v1/amenity-reports` (API.md:2897) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/auth/csrf` | `csrf` :111 | `csrf_api_v1_auth_csrf_get` | 200 MessageResponse | mention only — § 3.1 (API.md:380) |
| `POST /api/v1/auth/email/resend` | `resend_email` :196 | `resend_email_api_v1_auth_email_resend_post` | 200 MessageResponse | mention only — § 3.3 Email and password (API.md:404) |
| `POST /api/v1/auth/email/verify` | `verify_email` :185 | `verify_email_api_v1_auth_email_verify_post` | 200 MessageResponse | mention only — § 3.3 Email and password (API.md:404) |
| `GET /api/v1/auth/google/callback` | `google_callback` :149 | `google_callback_api_v1_auth_google_callback_get` | 307 free-form object | mention only — § 3.2 Google OAuth (API.md:387) |
| `GET /api/v1/auth/google/start` | `google_start` :144 | `google_start_api_v1_auth_google_start_get` | 307 free-form object | mention only — § 3.2 Google OAuth (API.md:387) |
| `POST /api/v1/auth/logout` | `logout` :281 | `logout_api_v1_auth_logout_post` | 200 MessageResponse | mention only — § 3.5 Session lifecycle (API.md:448) |
| `GET /api/v1/auth/methods` | `auth_methods` :102 | `auth_methods_api_v1_auth_methods_get` | **200 free-form object** | mention only — § 3. Authentication (API.md:370) |
| `GET /api/v1/auth/oauth/{provider}/callback` | `oauth_callback` :129 | `oauth_callback_api_v1_auth_oauth__provider__callback_get` | 307 free-form object | mention only — § 3.2 Google OAuth (API.md:387) |
| `GET /api/v1/auth/oauth/{provider}/start` | `oauth_start` :117 | `oauth_start_api_v1_auth_oauth__provider__start_get` | 307 free-form object | mention only — § 3.2 Google OAuth (API.md:387) |
| `POST /api/v1/auth/password/reset/complete` | `password_reset_complete` :234 | `password_reset_complete_api_v1_auth_password_reset_complete_post` | 200 MessageResponse | mention only — § 3.4 Password recovery (API.md:435) |
| `POST /api/v1/auth/password/reset/request` | `password_reset_request` :218 | `password_reset_request_api_v1_auth_password_reset_request_post` | 200 MessageResponse | mention only — § 3.4 Password recovery (API.md:435) |
| `POST /api/v1/auth/password/reset/verify` | `password_reset_verify` :227 | `password_reset_verify_api_v1_auth_password_reset_verify_post` | 200 MessageResponse | mention only — § 3.4 Password recovery (API.md:435) |
| `POST /api/v1/auth/password/sign-in` | `password_sign_in` :175 | `password_sign_in_api_v1_auth_password_sign_in_post` | 200 MessageResponse | mention only — § 3.3 Email and password (API.md:404) |
| `POST /api/v1/auth/password/sign-up` | `password_sign_up` :154 | `password_sign_up_api_v1_auth_password_sign_up_post` | 200 MessageResponse | mention only — § 3.3 Email and password (API.md:404) |
| `POST /api/v1/auth/refresh` | `refresh` :269 | `refresh_api_v1_auth_refresh_post` | 200 MessageResponse | mention only — § 3.5 Session lifecycle (API.md:448) |
| `GET /api/v1/auth/session` | `session` :245 | `session_api_v1_auth_session_get` | 200 SessionContext | mention only — § 3.5 Session lifecycle (API.md:448) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `POST /api/v1/complaints/admin-raise` | `admin_raise_complaint` :66 | `admin_raise_complaint_api_v1_complaints_admin_raise_post` | 201 AdminComplaintRaised | § `POST /api/v1/complaints/admin-raise` (API.md:1359) |
...
| `PATCH /api/v1/complaints/{complaint_id}` | `update_complaint` :124 | `update_complaint_api_v1_complaints__complaint_id__patch` | 200 MessageResult | § `PATCH /api/v1/complaints/{complaintId}` (API.md:1442) |
| `POST /api/v1/complaints/{complaint_id}/comments` | `add_comment` :151 | `add_comment_api_v1_complaints__complaint_id__comments_post` | 201 MessageResult | § `POST /api/v1/complaints/{complaintId}/comments` (API.md:1474) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `POST /api/v1/dashboard/amenities` | `create_amenity` :54 | `create_amenity_api_v1_dashboard_amenities_post` | **200 free-form object** | § `POST /api/v1/dashboard/amenities` · `PUT /api/v1/dashboard/amenities/{amenityId}` (API.md:2211) |
| `PUT /api/v1/dashboard/amenities/{amenity_id}` | `update_amenity` :71 | `update_amenity_api_v1_dashboard_amenities__amenity_id__put` | **200 free-form object** | § `POST /api/v1/dashboard/amenities` · `PUT /api/v1/dashboard/amenities/{amenityId}` (API.md:2211) |
| `DELETE /api/v1/dashboard/amenities/{amenity_id}` | `delete_amenity` :85 | `delete_amenity_api_v1_dashboard_amenities__amenity_id__delete` | **200 free-form object** | mention only — the POST/PUT heading covers `DELETE`'s error table too |
| `GET /api/v1/dashboard/events` | `dashboard_events` :32 | `dashboard_events_api_v1_dashboard_events_get` | 200 `text/event-stream` | mention only — § 5.1 (API.md:708) |
| `GET /api/v1/dashboard/snapshot` | `get_dashboard_snapshot` :17 | `get_dashboard_snapshot_api_v1_dashboard_snapshot_get` | 200 DashboardSnapshot | mention only — § 5 (API.md:663) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/geo/reverse` | `reverse_place` :70 | `reverse_place_api_v1_geo_reverse_get` | 200 GeoPlace | § `GET /api/v1/geo/reverse` (API.md:8754) |
| `GET /api/v1/geo/search` | `search_places` :31 | `search_places_api_v1_geo_search_get` | 200 array of GeoPlace | § `GET /api/v1/geo/search` (API.md:8725) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/departments` | `list_departments` :56 | `list_departments_api_v1_departments_get` | 200 Page_DepartmentDetail_ | § `GET /api/v1/departments` (API.md:1700) |
| `POST /api/v1/departments` | `create_department` :96 | `create_department_api_v1_departments_post` | 201 DepartmentDetail | § `POST /api/v1/departments` (API.md:1816) |
| `GET /api/v1/departments/{department_id}` | `get_department` :118 | `get_department_api_v1_departments__department_id__get` | 200 DepartmentDetail | § `GET /api/v1/departments/{departmentId}` (API.md:1860) |
| `PATCH /api/v1/departments/{department_id}` | `update_department` :132 | `update_department_api_v1_departments__department_id__patch` | 200 DepartmentDetail | § `PATCH /api/v1/departments/{departmentId}` (API.md:1903) |
| `DELETE /api/v1/departments/{department_id}` | `delete_department` :160 | `delete_department_api_v1_departments__department_id__delete` | 200 MessageResult | § `DELETE /api/v1/departments/{departmentId}` (API.md:1932) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/events` | `events` :78 | `events_api_v1_events_get` | 200 `text/event-stream` | § 5.1 Live updates — `GET /events` (API.md:708) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `POST /api/v1/admin/invitations` | `create_invitation` :20 | `create_invitation_api_v1_admin_invitations_post` | 200 InvitationCreated | § `POST /api/v1/admin/invitations` (API.md:602) |
| `POST /api/v1/invitations/prepare` | `prepare_invitation` :29 | `prepare_invitation_api_v1_invitations_prepare_post` | 200 MessageResponse | § `POST /api/v1/invitations/prepare` (API.md:641) |
| `POST /api/v1/invitations/redeem` | `redeem_invitation` :36 | `redeem_invitation_api_v1_invitations_redeem_post` | 200 MessageResponse | § `POST /api/v1/invitations/redeem` (API.md:651) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/billing-settings` | `get_billing_settings` :99 | `get_billing_settings_api_v1_billing_settings_get` | 200 BillingSettings | § `GET /api/v1/billing-settings` (API.md:2067) |
| `PUT /api/v1/billing-settings` | `update_billing_settings` :116 | `update_billing_settings_api_v1_billing_settings_put` | 200 BillingSettings | § `PUT /api/v1/billing-settings` (API.md:2119) |
| `POST /api/v1/invoices` | `create_invoice` :47 | `create_invoice_api_v1_invoices_post` | 201 InvoiceDetail | § `POST /api/v1/invoices` (API.md:1985) |
| `POST /api/v1/invoices/{invoice_id}/payments` | `record_payment` :71 | `record_payment_api_v1_invoices__invoice_id__payments_post` | 201 InvoiceDetail | § `POST /api/v1/invoices/{invoiceId}/payments` (API.md:2026) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `POST /api/v1/notices` | `create_notice` :26 | `create_notice_api_v1_notices_post` | 201 NoticeCreated | § 12.1 `POST /notices` (API.md:3234) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/notifications` | `list_notifications` :31 | `list_notifications_api_v1_notifications_get` | 200 NotificationFeed | § `GET /api/v1/notifications` (API.md:821) |
| `POST /api/v1/notifications/read-all` | `mark_all_notifications_read` :100 | `mark_all_notifications_read_api_v1_notifications_read_all_post` | 200 NotificationReadResult | § `POST /api/v1/notifications/read-all` (API.md:869) |
| `POST /api/v1/notifications/{notification_id}/read` | `mark_notification_read` :74 | `mark_notification_read_api_v1_notifications__notification_id__read_post` | 200 NotificationReadResult | § `POST /api/v1/notifications/{notificationId}/read` (API.md:860) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `POST /api/v1/onboarding/community` | `create_community` :12 | `create_community_api_v1_onboarding_community_post` | 200 CommunityOnboardingResponse | mention only — § 16.6 (API.md:4578) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `POST /api/v1/admins` | `promote_admin` :28 | `promote_admin_api_v1_admins_post` | 200 AdminSummary | § 12.2 `POST /admins` (API.md:3289) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `POST /api/v1/push/subscriptions` | `subscribe` :58 | `subscribe_api_v1_push_subscriptions_post` | 200 PushSubscriptionResult | § 5.3 Web Push (API.md:881) |
| `POST /api/v1/push/subscriptions/unregister` | `unsubscribe` :89 | `unsubscribe_api_v1_push_subscriptions_unregister_post` | 200 PushSubscriptionResult | § 5.3 Web Push (API.md:881) |
| `GET /api/v1/push/vapid-key` | `vapid_key` :33 | `vapid_key_api_v1_push_vapid_key_get` | 200 VapidPublicKey | § 5.3 Web Push (API.md:881) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/amenities/available` | `list_available_amenities` :30 | `list_available_amenities_api_v1_amenities_available_get` | 200 Page_BookableAmenity_ | § `GET /api/v1/amenities/available` (API.md:2245) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/complaints` | `list_my_complaints` :67 | `list_my_complaints_api_v1_complaints_get` | 200 Page_ComplaintSummary_ | § `GET /api/v1/complaints` (API.md:1511) |
| `POST /api/v1/complaints` | `raise_complaint` :112 | `raise_complaint_api_v1_complaints_post` | 201 ComplaintDetail | § `POST /api/v1/complaints` (API.md:1546) |
| `GET /api/v1/complaints/{complaint_id}` | `get_complaint` :151 | `get_complaint_api_v1_complaints__complaint_id__get` | 200 ComplaintDetail | § `GET /api/v1/complaints/{complaintId}` (API.md:1601) |
| `POST /api/v1/complaints/{complaint_id}/cancel` | `cancel_complaint_work` :184 | `cancel_complaint_work_api_v1_complaints__complaint_id__cancel_post` | 200 ComplaintDetail | § Complaint Engine v2 additions (API.md:61) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/directory/contacts` | `list_contacts` :111 | `list_contacts_api_v1_directory_contacts_get` | 200 array of ManagementContact | § `GET /api/v1/directory/contacts` (API.md:3760) |
| `GET /api/v1/me/household` | `get_household` :61 | `get_household_api_v1_me_household_get` | 200 array of HouseholdMember | § `GET /api/v1/me/household` (API.md:3728) |
| `POST /api/v1/me/household/phones` | `add_household_phone` :85 | `add_household_phone_api_v1_me_household_phones_post` | 200 array of HouseholdMember | § `POST /api/v1/me/household/phones` (API.md:3741) |
| `GET /api/v1/notices` | `list_notices` :31 | `list_notices_api_v1_notices_get` | 200 Page_Notice_ | § `GET /api/v1/notices` (API.md:3715) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/amenity-bookings/mine` | `list_my_bookings` :119 | `list_my_bookings_api_v1_amenity_bookings_mine_get` | 200 Page_ResidentBooking_ | § `GET /api/v1/amenity-bookings/mine` (API.md:3673) |
| `POST /api/v1/amenity-bookings/{booking_id}/pay` | `pay_booking` :154 | `pay_booking_api_v1_amenity_bookings__booking_id__pay_post` | 200 PaymentOutcome | § `POST /api/v1/amenity-bookings/{bookingId}/pay` (API.md:3694) |
| `GET /api/v1/invoices/mine` | `list_my_invoices` :38 | `list_my_invoices_api_v1_invoices_mine_get` | 200 Page_ResidentInvoice_ | § `GET /api/v1/invoices/mine` (API.md:3620) |
| `POST /api/v1/invoices/{invoice_id}/pay` | `pay_invoice` :76 | `pay_invoice_api_v1_invoices__invoice_id__pay_post` | 200 PaymentOutcome | § `POST /api/v1/invoices/{invoiceId}/pay` (API.md:3645) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/resident/snapshot` | `resident_snapshot` :28 | `resident_snapshot_api_v1_resident_snapshot_get` | 200 ResidentSnapshot | § `GET /api/v1/resident/snapshot` (API.md:3799) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/visitor-passes` | `list_my_visitor_passes` :34 | `list_my_visitor_passes_api_v1_visitor_passes_get` | 200 Page_VisitorPass_ | § `GET /api/v1/visitor-passes` (API.md:3398) |
| `POST /api/v1/visitor-passes` | `create_visitor_pass` :69 | `create_visitor_pass_api_v1_visitor_passes_post` | 201 VisitorPassCreated | § `POST /api/v1/visitor-passes` (API.md:3416) |
| `GET /api/v1/visitor-passes/{pass_id}` | `get_visitor_pass` :101 | `get_visitor_pass_api_v1_visitor_passes__pass_id__get` | 200 VisitorPass | § `GET /api/v1/visitor-passes/{passId}` (API.md:3465) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `POST /api/v1/service-providers` | `register` :100 | `register_api_v1_service_providers_post` | 201 ServiceProviderProfile | § `POST /api/v1/service-providers` (API.md:5094) |
| `GET /api/v1/service-providers/me` | `get_mine` :126 | `get_mine_api_v1_service_providers_me_get` | 200 ServiceProviderProfile | § `GET /api/v1/service-providers/me` (API.md:5143) |
| `PATCH /api/v1/service-providers/me` | `update_mine` :149 | `update_mine_api_v1_service_providers_me_patch` | 200 ServiceProviderProfile | § `PATCH /api/v1/service-providers/me` (API.md:5175) |
...
| `PUT /api/v1/service-providers/me/skills` | `set_skills` :230 | `set_skills_api_v1_service_providers_me_skills_put` | 200 SkillsSavedResult | § `PUT /api/v1/service-providers/me/skills` (API.md:5201) |
...
| `GET /api/v1/skills` | `list_skills` :46 | `list_skills_api_v1_skills_get` | 200 array of Skill | § `GET /api/v1/skills` (API.md:4907) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/worker/applications` | `list_my_applications` :103 | `list_my_applications_api_v1_worker_applications_get` | 200 array of ServiceApplication | § `GET /api/v1/worker/applications` (API.md:5427) |
| `POST /api/v1/worker/applications` | `apply` :131 | `apply_api_v1_worker_applications_post` | 201 ServiceApplication | § `POST /api/v1/worker/applications` (API.md:5443) |
| `DELETE /api/v1/worker/applications/{application_id}` | `withdraw` :157 | `withdraw_api_v1_worker_applications__application_id__delete` | 200 ServiceApplication | § `DELETE …/{applicationId}` |
| `POST /api/v1/worker/applications/{application_id}/decision` | `decide_invitation` :181 | `decide_invitation_api_v1_worker_applications__application_id__decision_post` | 200 ServiceApplication | § `POST …/{applicationId}/decision` |
| `GET /api/v1/worker/communities` | `list_my_communities` :43 | `list_my_communities_api_v1_worker_communities_get` | 200 array of ServiceEngagement | § `GET /api/v1/worker/communities` (API.md:5351) |
...
| `POST /api/v1/worker/communities/{staff_id}/departure` | `request_departure` :209 | `request_departure_api_v1_worker_communities__staff_id__departure_post` | 201 StaffDeparture | § `POST …/{staffId}/departure` |
| `DELETE /api/v1/worker/communities/{staff_id}/departure` | `cancel_departure` :252 | `cancel_departure_api_v1_worker_communities__staff_id__departure_delete` | 200 MessageResult | § `DELETE …/{staffId}/departure` |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/worker/jobs` | `list_jobs` :128 | `list_jobs_api_v1_worker_jobs_get` | 200 array of WorkerJob | § `GET /api/v1/worker/jobs` (API.md:7333) |
...
| `GET /api/v1/worker/open-jobs` | `list_open_jobs` :95 | `list_open_jobs_api_v1_worker_open_jobs_get` | 200 array of OpenJob | § `GET /api/v1/worker/open-jobs` (API.md:7382) |
| `GET /api/v1/worker/snapshot` | `worker_snapshot` :53 | `worker_snapshot_api_v1_worker_snapshot_get` | 200 WorkerSnapshot | § `GET /api/v1/worker/snapshot` (API.md:7275) |
...
| `GET /api/v1/worker/calendar` | `get_calendar` :40 | `get_calendar_api_v1_worker_calendar_get` | 200 array of CalendarEntry | § `GET /api/v1/worker/calendar` (API.md:7593) |
...
| `POST /api/v1/departments/{department_id}/applications/{application_id}/decide` | `decide` :167 | `decide_api_v1_departments__department_id__applications__application_id__decide_post` | 200 ServiceApplication | § `POST …/decide` |
| `POST /api/v1/departments/{department_id}/blacklist` | `blacklist` :238 | `blacklist_api_v1_departments__department_id__blacklist_post` | 201 MessageResult | § `POST …/blacklist` |
...
| `GET /api/v1/departments/{department_id}/departures` | `list_departures` :284 | `list_departures_api_v1_departments__department_id__departures_get` | 200 array of StaffDeparture | § `GET …/departures` |
| `POST /api/v1/departments/{department_id}/departures` | `open_departure` :338 | `open_departure_api_v1_departments__department_id__departures_post` | 201 StaffDeparture | § `POST …/departures` |
| `GET /api/v1/departments/{department_id}/departures/{departure_id}` | `get_departure` :311 | `get_departure_api_v1_departments__department_id__departures__departure_id__get` | 200 StaffDepartureDetail | § `GET …/departures/{departureId}` |
| `GET /api/v1/departments/{department_id}/departures/{departure_id}/coverage` | `get_departure_coverage` :514 | `get_departure_coverage_api_v1_departments__department_id__departures__departure_id__coverage_get` | 200 array of CoverageItem | § `GET …/coverage` |
| `POST /api/v1/departments/{department_id}/departures/{departure_id}/decide` | `decide_departure` :415 | `decide_departure_api_v1_departments__department_id__departures__departure_id__decide_post` | 200 StaffDeparture | § `POST …/decide` |
| `POST /api/v1/departments/{department_id}/departures/{departure_id}/reassign` | `reassign_item` :377 | `reassign_item_api_v1_departments__department_id__departures__departure_id__reassign_post` | 200 MessageResult | § `POST …/reassign` |
| `POST /api/v1/departments/{department_id}/invitations` | `invite` :131 | `invite_api_v1_departments__department_id__invitations_post` | 201 ServiceApplication | § `POST …/invitations` |
| `POST /api/v1/departments/{department_id}/members/{staff_id}/remove` | `remove_member` :205 | `remove_member_api_v1_departments__department_id__members__staff_id__remove_post` | 200 MessageResult | § `POST …/remove` |
| `GET /api/v1/departments/{department_id}/staff-invitations` | `list_staff_invitations` :560 | `list_staff_invitations_api_v1_departments__department_id__staff_invitations_get` | 200 array of StaffInvitation | **missing** |
| `POST /api/v1/departments/{department_id}/staff-invitations` | `invite_staff_member` :589 | `invite_staff_member_api_v1_departments__department_id__staff_invitations_post` | 201 StaffInvitation | **missing** |
| `PATCH /api/v1/departments/{department_id}/staff-invitations/{invitation_id}` | `update_staff_invitation` :626 | `update_staff_invitation_api_v1_departments__department_id__staff_invitations__invitation_id__patch` | 200 StaffInvitation | **missing** |
| `DELETE /api/v1/departments/{department_id}/staff-invitations/{invitation_id}` | `revoke_staff_invitation` :670 | `revoke_staff_invitation_api_v1_departments__department_id__staff_invitations__invitation_id__delete` | 200 MessageResult | **missing** |
| `GET /api/v1/departments/{department_id}/staff/{staff_id}` | `get_staff_member` :451 | `get_staff_member_api_v1_departments__department_id__staff__staff_id__get` | 200 StaffMemberDetail | § `GET …/staff/{staffId}` |
| `GET /api/v1/departments/{department_id}/staff/{staff_id}/schedule` | `get_staff_schedule` :478 | `get_staff_schedule_api_v1_departments__department_id__staff__staff_id__schedule_get` | 200 array of ScheduleItem | § `GET …/schedule` |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/conversations` | `list_conversations` :41 | `list_conversations_api_v1_conversations_get` | 200 array of Conversation | § `GET /api/v1/conversations` (API.md:6460) |
| `POST /api/v1/conversations` | `open_conversation` :69 | `open_conversation_api_v1_conversations_post` | 201 ConversationThread | § `POST /api/v1/conversations` (API.md:6493) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/messages/recipients` | `list_recipients` :33 | `list_recipients_api_v1_messages_recipients_get` | 200 array of DmRecipient | §20 `GET /api/v1/messages/recipients` (API.md:8579) |
| `GET /api/v1/messages/threads` | `list_threads` :59 | `list_threads_api_v1_messages_threads_get` | 200 array of DmThread | §20 `GET /api/v1/messages/threads` (API.md:8579) |
| `POST /api/v1/messages/threads` | `open_thread` :78 | `open_thread_api_v1_messages_threads_post` | 201 DmThreadDetail | §20 `POST /api/v1/messages/threads` (API.md:8579) |
| `GET /api/v1/messages/threads/{thread_id}` | `get_thread` :104 | `get_thread_api_v1_messages_threads__thread_id__get` | 200 DmThreadDetail | §20 `GET …/{threadId}` (API.md:8579) |
| `POST /api/v1/messages/threads/{thread_id}/messages` | `post_message` :128 | `post_message_api_v1_messages_threads__thread_id__messages_post` | 201 DmMessage | §20 `POST …/messages` (API.md:8579) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/complaints/{complaint_id}/work-orders` | `list_complaint_work_orders` :105 | `list_complaint_work_orders_api_v1_complaints__complaint_id__work_orders_get` | 200 array of WorkOrder | § `GET …/work-orders` |
| `POST /api/v1/complaints/{complaint_id}/work-orders` | `create_work_order` :57 | `create_work_order_api_v1_complaints__complaint_id__work_orders_post` | 201 WorkOrder | § `POST …/work-orders` |
| `GET /api/v1/departments/{department_id}/work-orders` | `list_department_work_orders` :123 | `list_department_work_orders_api_v1_departments__department_id__work_orders_get` | 200 array of WorkOrder | § `GET /api/v1/departments/{departmentId}/work-orders` (API.md:6791) |
| `GET /api/v1/work-orders/{work_order_id}` | `get_work_order` :147 | `get_work_order_api_v1_work_orders__work_order_id__get` | 200 WorkOrderDetail | § `GET /api/v1/work-orders/{workOrderId}` (API.md:6809) |
| `PATCH /api/v1/work-orders/{work_order_id}` | `update_work_order` :187 | `update_work_order_api_v1_work_orders__work_order_id__patch` | 200 WorkOrder | § `PATCH /api/v1/work-orders/{workOrderId}` (API.md:6852) |
| `POST /api/v1/work-orders/{work_order_id}/assign` | `assign_work_order` :213 | `assign_work_order_api_v1_work_orders__work_order_id__assign_post` | 200 WorkOrderDetail | § `POST …/assign` |
| `POST /api/v1/work-orders/{work_order_id}/cancel` | `cancel_work_order` :332 | `cancel_work_order_api_v1_work_orders__work_order_id__cancel_post` | 200 WorkOrder | § `POST …/cancel` |
| `GET /api/v1/work-orders/{work_order_id}/candidates` | `work_order_candidates` :172 | `work_order_candidates_api_v1_work_orders__work_order_id__candidates_get` | 200 array of Candidate | § Complaint Engine v2 additions (API.md:61) |
| `POST /api/v1/work-orders/{work_order_id}/reschedule` | `reschedule_work_order` :306 | `reschedule_work_order_api_v1_work_orders__work_order_id__reschedule_post` | 200 WorkOrderDetail | § `POST …/reschedule` |
| `POST /api/v1/work-orders/{work_order_id}/take-up` | `take_up_work_order` :259 | `take_up_work_order_api_v1_work_orders__work_order_id__take_up_post` | 200 WorkOrderDetail | § `POST …/take-up` |
...
| `POST /api/v1/security/gate/verify` | `verify_credential` :547 | `verify_credential_api_v1_security_gate_verify_post` | 200 GateVerification | § `POST /api/v1/security/gate/verify` (API.md:8396) |
...
| `GET /api/v1/security/posts` | `list_posts` :99 | `list_posts_api_v1_security_posts_get` | 200 array of SecurityPost | § `GET /api/v1/security/posts` (API.md:8070) |
| `POST /api/v1/security/posts` | `create_post` :114 | `create_post_api_v1_security_posts_post` | 201 SecurityPost | § `POST /api/v1/security/posts` (API.md:8089) |
...
| `GET /api/v1/security/roster` | `list_roster` :262 | `list_roster_api_v1_security_roster_get` | 200 array of RosterEntry | § `GET /api/v1/security/roster` (API.md:8191) |
| `GET /api/v1/security/shifts` | `list_shifts` :166 | `list_shifts_api_v1_security_shifts_get` | 200 array of SecurityShift | § `GET /api/v1/security/shifts` (API.md:8118) |
| `POST /api/v1/security/shifts` | `create_shift` :204 | `create_shift_api_v1_security_shifts_post` | 201 SecurityShift | § `POST /api/v1/security/shifts` (API.md:8145) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/complaint-categories` | `list_categories` :79 | `list_categories_api_v1_complaint_categories_get` | 200 array of ComplaintCategory | § `GET /api/v1/complaint-categories` (API.md:4973) |
| `GET /api/v1/departments/{department_id}/skills` | `list_department_skills` :104 | `list_department_skills_api_v1_departments__department_id__skills_get` | 200 array of Skill | § `GET /api/v1/departments/{departmentId}/skills` (API.md:5009) |
| `POST /api/v1/departments/{department_id}/skills` | `add_department_skill` :147 | `add_department_skill_api_v1_departments__department_id__skills_post` | 201 SkillCreated | § `POST /api/v1/departments/{departmentId}/skills` (API.md:5054) |
| `PUT /api/v1/departments/{department_id}/skills` | `set_department_skills` :123 | `set_department_skills_api_v1_departments__department_id__skills_put` | 200 array of Skill | § `PUT /api/v1/departments/{departmentId}/skills` (API.md:5028) |
| `DELETE /api/v1/departments/{department_id}/skills/{skill_id}` | `remove_department_skill` :178 | `remove_department_skill_api_v1_departments__department_id__skills__skill_id__delete` | 200 MessageResult | § `DELETE /api/v1/departments/{departmentId}/skills/{skillId}` (API.md:5079) |
| `POST /api/v1/skills` | `create_skill` :47 | `create_skill_api_v1_skills_post` | 201 SkillCreated | § `GET /api/v1/skills` (API.md:4907) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `POST /api/v1/complaints/{complaint_id}/chat` | `open_complaint_chat` :259 | `open_complaint_chat_api_v1_complaints__complaint_id__chat_post` | 200 ComplaintThreadOpened | § `POST /api/v1/complaints/{complaintId}/chat` (API.md:7950) |
| `POST /api/v1/complaints/{complaint_id}/notes` | `add_complaint_note` :226 | `add_complaint_note_api_v1_complaints__complaint_id__notes_post` | 201 MessageResult | § `POST /api/v1/complaints/{complaintId}/notes` (API.md:7929) |
| `POST /api/v1/complaints/{complaint_id}/priority-raise` | `raise_complaint_priority` :186 | `raise_complaint_priority_api_v1_complaints__complaint_id__priority_raise_post` | 200 MessageResult | § `POST /api/v1/complaints/{complaintId}/priority-raise` (API.md:7899) |
| `POST /api/v1/complaints/{complaint_id}/resolve` | `resolve_complaint` :148 | `resolve_complaint_api_v1_complaints__complaint_id__resolve_post` | 200 MessageResult | § `POST /api/v1/complaints/{complaintId}/resolve` (API.md:7871) |
| `POST /api/v1/complaints/{complaint_id}/take-up` | `take_up_complaint` :105 | `take_up_complaint_api_v1_complaints__complaint_id__take_up_post` | 200 MessageResult | § `POST /api/v1/complaints/{complaintId}/take-up` (API.md:7788) |
| `GET /api/v1/departments/{department_id}/triage-snapshot` | `triage_snapshot` :60 | `triage_snapshot_api_v1_departments__department_id__triage_snapshot_get` | 200 TriageSnapshot | § `GET /api/v1/departments/{departmentId}/triage-snapshot` (API.md:7733) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `POST /api/v1/telemetry/service-signup` | `record_service_signup_event` :16 | `record_service_signup_event_api_v1_telemetry_service_signup_post` | 200 MessageResponse | § `POST /api/v1/telemetry/service-signup` (API.md:5512) |
...
| Operation | Handler | `operationId` (yaml anchor) | Success schema | API.md |
|---|---|---|---|---|
| `GET /api/v1/settings` | `get_settings` :57 | `get_settings_api_v1_settings_get` | 200 SettingsSnapshot | § `GET /api/v1/settings` (API.md:3077) |
| `PUT /api/v1/settings` | `update_settings` :76 | `update_settings_api_v1_settings_put` | 200 SettingsSnapshot | § `PUT /api/v1/settings` (API.md:3166) |
...
missing: all **211** operations are in the spec, with correct paths, methods, request bodies and
status codes. What is wrong is the *body* description on eight of them, and the *prose coverage* on
two. *(Read 179 until 2026-08-12, 195 until 2026-08-20 and 199 until 2026-08-24; the surface moved
four times while the findings did not.)*

**Every one of these is inherited, and none is on a surface this branch built.** That is not a
boast — it is the reason the count barely moved in three months of adding endpoints, and the
reason a *new* finding is worth stopping for.

> **Twenty → ten, 2026-08-28 — the first time the count moved by fixes rather than by surface.**
> The residence-claim session closed the two oldest findings at once: the three admin decision
> routes gained `AccessRequestDecisionResponse` (out of §5.1's untyped-body table), and `API.md` §6
> gained a `###` section for every one of the seven access-request operations (the five-strong
> access-request row out of §5.2). Both had stood since this file was created on 2026-08-08. The
> scan now reports **10**: the eight untyped bodies and the two `/communities` reads below.
...
| ~~`POST /admin/access-requests/{id}/approve` · `/reject` · `/blacklist`~~ | ~~200 free-form object~~ | `AccessRequestDecisionResponse` | **Closed 2026-08-28.** The residence-claim session gave all three routes a `response_model` — `{request_id, status, membership_id?, unit_id?}` — retiring the `-> dict` annotations this row had flagged since 2026-08-08. Kept as a row rather than deleted because the finding's age is the finding: it took a product change that needed the approve body (`unit_code`/`building_code`) for the response side to get typed too |
| `POST /dashboard/amenities`, `PUT` and `DELETE /dashboard/amenities/{id}` | 200 free-form object | The raw DB row; `DELETE` returns `{"id": …}` | **Response still under-documented**, and the row **differs by deployment** — `dashboard_repository` builds a different shape when `schema_generation() == "legacy"`. There is no single true schema to document until that branch is resolved. The **request** side gained coverage 2026-08-23 (issue #48 D2 — `AmenityWrite`'s `image`/`opening_time`/`closing_time`, API.md:2043), which is why §5.2 below no longer lists `PUT`/`DELETE` as having no reference section at all |
...
Two operations have no reference section. API.md's header states that every `###` heading
...
|---|---|---|
| ~~Access requests — `GET /access-requests/mine`, `POST /access-requests/{id}/withdraw`, and the admin `approve` · `reject` · `blacklist`~~ | ~~5~~ | **Closed 2026-08-28** — API.md §6 now carries a `###` section for all seven access-request operations (§6.1 and below), written in the pass that added the residence claim. The "one prose sentence in § 6. People" this row used to point at was itself rewritten to say what it had been hiding |
...
>
> *(Nine → seven, 2026-08-23: `PUT` and `DELETE /dashboard/amenities/{id}` dropped off this table —
> not because either handler changed, but because the amenity-defects fix gave `POST`/`PUT` a real
> request-body section (API.md:2043) that `DELETE`'s error table now hangs off too. The response body
> the three share is still undocumented; that gap lives in §5.1 above, not here.)*
...
*(And **10** as of 2026-08-28, against 211 operations — the first downward move, and by fixes
rather than by surface: the three access-request decision routes got typed and the five
undocumented access-request operations got their sections, closing eight findings in one pass. See
the dated notes in §5.)*
...
| Date | Change |
|---|---|
| 2026-08-28 | **Zero operations added, and the finding count moved for the first time by being fixed: 18 → 10.** The residence-claim session (`20260828090000_residence_claim_on_join.sql`) closed this file's two oldest findings in one pass — the §5.1 `-> dict` row (approve · reject · blacklist now answer `AccessRequestDecisionResponse`) and the §5.2 access-request row (all seven operations now hold a `###` section in `API.md` §6, which had said "intentionally empty" since 2026-07-30). The seven rows in §3's `access_requests.py` table were regenerated with their new success schemas and `API.md` anchors — the five `**missing**` cells and the two `mention only` cells were **blanked by hand first**, because `regen_mapper.py` deliberately preserves an existing editorial cell, `**missing**` included; a cleared cell is what lets it derive the link from the new heading. Also regenerated in the same run: the approve operation's spec entry (new `unit_code`/`building_code` body fields, 422, and the request/response fields on `POST /access-requests` — surface unchanged at **211 across 180 paths**). The tally said "132 row(s) differ", which per the 2026-08-20 row's warning is line-shift noise around eight real row changes |
| 2026-08-24 | **One operation added, and the tables had two reasons to move rather than one.** `POST /work-orders/{workOrderId}/take-up` (ruling R8, supervisor take-up) arrives with a `WorkOrderDetail` response model, an `API.md` section and one story tag (`US-2.8`), so the finding count is unchanged at **18** and the surface reads **211 operations across 180 paths**. Both kinds of drift this file exists to catch happened in the same commit: the new `API.md` section sits inside §18 and pushed ~50 line references below it down, and the new route pushed `reschedule_work_order` and `cancel_work_order` down inside `work_orders.py`, so **52 rows' derived cells moved as well as the references**. `regen_mapper.py` resynced both; nothing was hand-written except this row and the header paragraph. Worth recording: the row **was** briefly hand-added, because `regen_mapper.py` drops a row it cannot find in the live app — and it could not find this one until `openapi.yaml` was actually rewritten. The generated pair is `export_openapi.py` **then** `regen_mapper.py`, in that order, and running them the other way round looks exactly like a route that does not exist |
