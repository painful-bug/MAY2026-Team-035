...
>
> **That "nothing" expired the next day.** Ten sections have been written since
> — §23 through §28 dated 2026-08-23, §29–§30 dated 2026-08-23/24, §31 dated
> 2026-08-26, §33 dated 2026-08-28 — each a file in
...
> `20260823170000`, §28 `20260823180000`, §29 `20260823190000`, §30
> `20260824090000`, §31 `20260826090000`, §33 `20260828090000`. §27 through §30
> each state a dependency on the section before them (§29 and §30 re-issue their
> predecessors' function bodies); §31 is independent of all of them and sorts
> after by filename alone; §33 touches none of their objects and depends only on
> the long-applied §23-era state, but comes last because filename order is apply
> order. Filename order satisfies all of it, as it always does. Each section
> carries its own ledger insert and its own post-checks, and each says for
> itself what the last ledger probe found — this paragraph deliberately does not
> restate that, because a second copy of an apply status is the copy that goes
> stale. **§32 is deliberately absent from this copy**: the `live-app-fixes`
> branch holds an uncommitted section for
> `20260827210000_one_live_job_per_complaint.sql` numbered §32, and this file
> leaves that number free so the merge that brings it needs no renumbering. Its
> migration sorts between §31's and §33's, touches neither's objects, and slots
> into the same filename-order rule. **§33 is the newest in this copy.** If the
> section numbers below run past §33, this paragraph is what needs updating.
>
...
**Apply this AFTER §31 (and after `20260827210000_one_live_job_per_complaint.sql`,
the reserved §32, if that file has reached this copy).** Not because they share
an object — this file touches `access_requests`, `approve_access_request` and
`pending_access_request_overview`, none of which §29, §30, §31 or the §32
