...
>
> **That "nothing" expired the next day.** Nine sections have been written since
> — §23 through §28 dated 2026-08-23, §29–§30 dated 2026-08-23/24, §31 dated
> 2026-08-28 — each a file in `backend/supabase/migrations/`. **Whichever of
> them are still outstanding, apply in filename order**, which is section order
> here: §23 `20260823120000`, §24 `20260823150000`, §25 `20260823153000`, §26
> `20260823160000`, §27 `20260823170000`, §28 `20260823180000`, §29
> `20260823190000`, §30 `20260824090000`, §31 `20260828090000`. §27 through §30
> each state a dependency on the section before them (§29 and §30 re-issue their
> predecessors' function bodies); §31 touches none of their objects and depends
> only on the long-applied §23-era state, but comes after §30 because filename
> order is apply order. Filename order satisfies all of it, as it always does.
> Each section carries its own ledger insert and its own post-checks, and each
> says for itself what the last ledger probe found — this paragraph deliberately
> does not restate that, because a second copy of an apply status is the copy
> that goes stale. (Two parallel workstreams on the `live-app-fixes` branch hold
> migrations `20260826090000_realtime_expansion.sql` and
> `20260827210000_one_live_job_per_complaint.sql`, whose runbook copy numbers
> them §31 and §32; the section numbers will be reconciled at merge. This
> branch's file was deliberately timestamped `20260828090000` to sort after
> both, and none of the three files touches another's objects, so filename
> order — theirs, then this one — is safe and is the order to apply.) **§31 is the newest in this copy.** If the section numbers below run
> past §31, this paragraph is what needs updating.
>
...
## 31. `20260828090000_residence_claim_on_join.sql`

**Apply this AFTER §30.** Not because they share an object — this file touches
`access_requests`, `approve_access_request` and `pending_access_request_overview`,
none of which §29 or §30 name — but because filename order is apply order and
this file's version sorts after §30's. If §30 is somehow still outstanding when
you get here, applying this one first breaks nothing; apply §30 anyway, and
apply both.
